  ...
  const sendMessage = useCallback(
    async (text: string, bypassPii = false, overrideFile?: { name: string; content: string } | null, anonMappings?: { original: string; anonymized: string }[]) => {
      ...
      try {
        ...
        // ── Auto-Deanonymize (after streaming, if anonymized content was sent) ──
        if (anonMappings && autoDeanonymize && fullText) {
          setMessages((prev) =>
            prev.map((m) =>
              m.id === assistantId ? { ...m, deanonymized: { text: "", checking: true } } : m
            )
          );
          ...
          try {
            ...
            if (dRes.ok && dData.deanonymized_text) {
              ...
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === assistantId ? { ...m, deanonymized: undefined } : m
                )
              );
            }
          } catch {
    ...
    [isLoading, messages, context, customerId, uploadedFile, safetySettings, autoDeanonymize]
  );
          ...
          <div key={message.id}>
            ...
            >
              ...
              >
                ...
                {/* Deanonymize status indicator */}
                {message.role === "assistant" && !message.isStreaming && message.anonymizeMappings && message.id !== "welcome" && message.deanonymized?.checking && (
                  <div className="mt-2 text-xs px-2.5 py-1.5 rounded-lg font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400">
                    <span className="inline-flex items-center gap-1.5">Deanonymizing response<span className="inline-flex gap-0.5 ml-0.5"><span className="w-1 h-1 rounded-full bg-current animate-bounce" style={{ animationDelay: "0ms" }} /><span className="w-1 h-1 rounded-full bg-current animate-bounce" style={{ animationDelay: "150ms" }} /><span className="w-1 h-1 rounded-full bg-current animate-bounce" style={{ animationDelay: "300ms" }} /></span></span>
                  </div>
                )}
              </div>
            </div>
