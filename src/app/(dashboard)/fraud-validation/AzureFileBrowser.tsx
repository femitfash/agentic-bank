...
export default function AzureFileBrowser({ open, onClose, onFileSelected }: AzureFileBrowserProps) {
  ...
  async function fetchContainers() {
    ...
    try {
      ...
      if (formAuth === "entra") {
        reqBody.account = formAccount.trim();
      } else {
        reqBody.conn = formConnStr.trim();
      }
      const res = await fetch("/api/admin/fraud-validation/azure-browse", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok) {
        setContainerError(data.error || "Failed to list containers");
        return;
      }
      if (data.containers && data.containers.length > 0) {
        ...
        setContainerError("No containers found in this storage account");
      }
    } catch {
      setContainerError("Network error while listing containers");
    } finally {
